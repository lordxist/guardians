...
describe Base do  
  it "at takes two integers as parameters and returns its kind" do
    Base.at_coords(0, 0).should be_kind_of(Base)
  end
  ...
  it "has an x_coord set correctly" do
    Base.at_coords(0, 1).x_coord.should eql(0)
    Base.at_coords(1, 0).x_coord.should eql(1)
  end
  
  it "has a y_coord set correctly" do
    Base.at_coords(0, 1).y_coord.should eql(1)
    Base.at_coords(1, 0).y_coord.should eql(0)
  end
  
  describe "dynamic object(s) at same position" do
    before(:each) do
      Object.stubs(:at_coords).returns(["test"])
    end
    
    it "(plural) returns the correct list" do
      Base.at_coords(0, 0).objects.should eql(["test"])
    end
    
    it "(singular) returns a single object" do
      Base.at_coords(0, 0).object.should eql("test")
    end
  end
end
